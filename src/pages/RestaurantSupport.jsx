...
import {
    ...
    Send, ChevronRight, HelpCircle, ShoppingCart,
    CreditCard, Users, Settings, Star, Bot, Store
} from "lucide-react";
...
// Restaurant-specific issue categories
const issueCategories = [
    { id: "orders", label: "Order Issues", icon: ShoppingCart, color: "bg-blue-100 text-blue-600" },
    { id: "payout", label: "Payout & Commission", icon: CreditCard, color: "bg-green-100 text-green-600" },
    { id: "menu", label: "Menu Management", icon: Settings, color: "bg-purple-100 text-purple-600" },
    { id: "reviews", label: "Customer Reviews", icon: Star, color: "bg-yellow-100 text-yellow-600" },
    { id: "customers", label: "Customer Dispute", icon: Users, color: "bg-orange-100 text-orange-600" },
    { id: "account", label: "Account Settings", icon: Store, color: "bg-red-100 text-red-600" },
...
export default function RestaurantSupport() {
    ...
    const checkAuth = async () => {
        try {
            ...
            // Only restaurants can access this page
    ...
    const createTicketMutation = useMutation({
        mutationFn: async (ticketData) => {
            return base44.entities.SupportTicket.create({
                ...
                customer_name: restaurant?.name || 'Restaurant Partner',
                user_type: 'restaurant',
                status: 'open',
                priority: ticketData.category === 'payout' ? 'high' : 'medium',
                messages: [{
                    sender: 'restaurant',
        ...
        onSuccess: () => {
            queryClient.invalidateQueries(['restaurant-support-tickets']);
            setShowNewTicket(false);
            setFormData({ category: "", subject: "", description: "" });
            toast.success("Ticket created! Our partner support team will respond soon.");
        },
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold">Partner Support</h1>
                    <p className="text-gray-500">Get help with orders, payouts, and account management</p>
                </div>
                <Button
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {issueCategories.slice(0, 4).map(cat => (
                    <button
                        ...
                        className="bg-white p-4 rounded-xl border-2 border-transparent hover:border-purple-500 transition-all text-left"
                    >
            ...
            {/* Partner Resources */}
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-2xl p-4 mb-6 text-white">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-semibold">Partner Resources</p>
                            <p className="text-sm text-white/80">FAQs, guides, and best practices</p>
                        </div>
                    </div>
                    <Button variant="secondary" className="bg-white text-purple-600 hover:bg-purple-50 rounded-xl">
                        View Resources
                    </Button>
                </div>
            </div>
                            ...
                            <div className="space-y-4 max-h-[300px] overflow-y-auto p-4 bg-gray-50 rounded-xl">
                                {selectedTicket.messages?.map((msg, idx) => (
                                    <div
                                        ...
                                        className={cn(
                                            "flex gap-3",
                                            msg.sender === 'restaurant' ? "flex-row-reverse" : ""
                                        )}
                                    >
                                        <div className={cn(
                                            "w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0",
                                            msg.sender === 'restaurant' ? "bg-purple-600" : "bg-blue-600"
                                        )}>
                                        ...
                                        <div className={cn(
                                            ...
                                            msg.sender === 'restaurant' ? "bg-purple-600 text-white" : "bg-white"
                                        )}>
                                            <p className="text-sm">{msg.message}</p>
                                            <p className={cn(
                                                "text-xs mt-1",
                                                msg.sender === 'restaurant' ? "text-purple-200" : "text-gray-400"
                                            )}>
