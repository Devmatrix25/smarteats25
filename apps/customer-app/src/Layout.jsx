...
export default function Layout({ children, currentPageName }) {
  ...
  const loadUser = async () => {
    try {
      ...
      // Check if we need role-based redirect after login
      const pendingRedirect = sessionStorage.getItem('pending_role_redirect');
      if (pendingRedirect) {
        sessionStorage.removeItem('pending_role_redirect');
  ...
  const loadCart = async () => {
    try {
      ...
      if (isAuth) {
        const userData = await base44.auth.me();
        const carts = await base44.entities.Cart.filter({ customer_email: userData.email });
  ...
  const isCustomerPage = !isAdminPage && !isRestaurantPage && !isDriverPage;

  // Hide layout for certain pages (Index is the landing page without layout)
  const hideLayout = ['Login', 'Register', 'Index'].includes(currentPageName);
  if (hideLayout) return children;

  // Check if user is authenticated for protected pages
  if (!user && isCustomerPage && currentPageName !== 'Index') {
    // Still loading, show nothing while checking auth
    return null;
  }
