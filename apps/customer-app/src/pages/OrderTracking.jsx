...
export default function OrderTracking() {
  ...
  const { data: order, isLoading, refetch } = useQuery({
    ...
    refetchInterval: 1500 // Poll every 1.5 seconds for true real-time updates
  });
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge className={cn(
              ...
              order.order_status === 'scheduled' && "bg-purple-100 text-purple-700",
              !['delivered', 'cancelled', 'scheduled'].includes(order.order_status) && "bg-orange-100 text-orange-700"
            )}>
                ...
                {statusSteps.map((step, idx) => {
                  ...
                  return (
                    <div key={step.key} className="text-center">
                      <div className={cn(
                        "w-8 h-8 rounded-full mx-auto mb-2 flex items-center justify-center transition-all",
                        isCompleted ? "bg-[#3BA55D] text-white" : "bg-gray-100 text-gray-400",
                        isCurrent && "ring-4 ring-[#3BA55D]/20 scale-110"
                      )}>
                        <StepIcon className="w-4 h-4" />
                      </div>
                      <p className={cn(
                        "text-xs",
                        isCompleted ? "text-[#3BA55D] font-medium" : "text-gray-400"
                      )}>
                        {step.label}
                      </p>
                    </div>
          ...
          {/* ETA */}
          {order.estimated_delivery_time && !['delivered', 'cancelled'].includes(order.order_status) && (
            <div className="mt-6 p-4 bg-orange-50 rounded-xl flex items-center gap-3">
              <Clock className="w-5 h-5 text-[#F25C23]" />
              <div>
                <p className="font-medium text-[#1D1D1F]">Estimated Delivery</p>
